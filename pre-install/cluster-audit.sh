...
# probe for disk info ###############
clush $parg2 "echo 'Storage Controller: '; ${SUDO:-} lspci | grep -i -e ide -e raid -e storage -e lsi"; echo $sep
clush $parg2 "echo 'Udev rules: '; ${SUDO:-} ls /etc/udev/rules.d"; echo $sep
...
clush $parg2 'for each in  /sys/block/{s,xv}d*/queue/max_hw_sectors_kb; do printf "%s: %s\n" $each $(cat $each); done'; echo $sep
clush $parg2 'for each in  /sys/block/{s,xv}d*/queue/max_sectors_kb; do printf "%s: %s\n" $each $(cat $each); done'; echo $sep
