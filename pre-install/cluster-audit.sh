...
clush $parg2 "${SUDO:-} dmidecode | grep -A3 '^BIOS I'" 2> /dev/null; echo $sep
...
clush $parg2 "echo -n 'DIMM slots: '; ${SUDO:-} dmidecode |grep -c '^[[:space:]]*Locator:'" 2> /dev/null; echo $sep
clush $parg2 "echo -n 'DIMM count is: '; ${SUDO:-} dmidecode | grep -c '^[[:space:]]Size: [0-9]* MB'" 2> /dev/null; echo $sep
clush $parg2 "${SUDO:-} dmidecode | awk '/Memory Device$/,/^$/ {print}' | grep -e '^Mem' -e Size: -e Speed: -e Part | sort -u | grep -v -e 'NO DIMM' -e 'No Module Installed' -e Unknown" 2> /dev/null; echo $sep
...
clush $parg2 "${SUDO:-} lspci | grep -i ether" 2> /dev/null
clush $parg2 "${SUDO:-} ip link show | sed '/ lo: /,+1d' | awk '/UP/{sub(\":\",\"\",\$2);print \$2}' | xargs -l ${SUDO:-} ethtool | grep -e ^Settings -e Speed" 2> /dev/null
#clush $parg "echo -n 'Nic Speed: '; /sbin/ip link show | sed '/ lo: /,+1d' | awk '/UP/{sub(\":\",\"\",\$2);print \$2}' | xargs -l -I % cat /sys/class/net/%/speed"
echo $sep

# probe for disk info ###############
clush $parg2 "echo 'Storage Controller: '; ${SUDO:-} lspci | grep -i -e raid -e storage -e lsi" 2> /dev/null; echo $sep
clush $parg "dmesg | grep -i raid | grep -i scsi"; echo $sep
case $distro in
   ubuntu)
	clush $parg2 "${SUDO:-} fdisk -l | grep '^Disk /.*:'" 2> /dev/null; echo $sep
   ;;
   redhat|centos|red*)
...
clush $parg2 "${SUDO:-} sysctl vm.swappiness net.ipv4.tcp_retries2 vm.overcommit_memory" 2> /dev/null; echo $sep
...
case $distro in
   ubuntu)
      ...
      clush $parg2 "${SUDO:-} apparmor_status | sed 's/([0-9]*)//'" 2> /dev/null; echo $sep
      ...
      clush $parg2 "echo 'Firewall status: '; ${SUDO:-} service ufw status | head -10" 2> /dev/null; echo $sep
      clush $parg2 "echo 'IPtables status: '; ${SUDO:-} iptables -L | head -10" 2> /dev/null; echo $sep
      clush $parg2 'echo "NTP status "; ${SUDO:-} service ntp status' 2> /dev/null; echo $sep
   ...
   redhat|centos|red*)
      ...
      clush $parg2 "${SUDO:-} chkconfig --list iptables"  2> /dev/null; echo $sep
      #clush $parg "/sbin/service iptables status | grep -m 3 -e ^Table -e ^Chain" 
      clush $parg2 "${SUDO:-} service iptables status | head -10" 2> /dev/null; echo $sep
      ...
      clush $parg2 "echo -n 'CPUspeed Service: '; ${SUDO:-} service cpuspeed status" 2> /dev/null 
      clush $parg2 "echo -n 'CPUspeed Service: '; ${SUDO:-} chkconfig --list cpuspeed" 2> /dev/null; echo $sep
...
clush -B $parg 'java -XX:+PrintFlagsFinal -version |& grep MaxHeapSize'; echo $sep
echo All IP addresses for hostname
...
clush $parg2 "grep -h nproc /etc/security/limits.d/*nproc.conf" 2>/dev/null
clush $parg2 "echo -n 'Open process limit(should be >=32K): '; ${SUDO:-} su - mapr -c 'ulimit -u'"  2> /dev/null; echo $sep
echo Check for mapr user open file limit
clush $parg2 "echo -n 'Open file limit(should be >=32K): '; ${SUDO:-} su - mapr -c 'ulimit -n'"  2> /dev/null; echo $sep
echo Check for mapr users java exec permission and version
clush $parg2 "echo -n 'Jave version: '; ${SUDO:-} su - mapr -c 'java -version'" 2> /dev/null; echo $sep
echo "Check for mapr user login and passwordless ssh (only for MapR v3.x)"
clush $parg2 "if getent passwd mapr; then ${SUDO:-} ls ~mapr/.ssh; else echo 'mapr user NOT found'; fi" 2>/dev/null; echo $sep
echo "Check for root user login and passwordless ssh (not needed for MapR, just easy for clush)"
clush $parg2 "echo 'Root login '; getent passwd root && { ${SUDO:-} echo ~root/.ssh; ${SUDO:-} ls ~root/.ssh; }" 2> /dev/null; echo $sep
