...
clush -B -g zkcldb 'yum -y install mapr-zookeeper' #3 Zookeeper nodes, fileserver, tt and nfs could be erased
...
ssh $node1 service mapr-warden start  # Start 1 CLDB and webserver on first node
echo Wait at least 90 seconds for system to initialize; sleep 90

ssh $node1 maprcli node cldbmaster && ssh $node1 maprcli acl edit -type cluster -user mapr:fc
