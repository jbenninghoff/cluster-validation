...
# One half of nodes (clients) sends load to other half (servers) and measures throughput
# MapR rpctest is a client/server binary, use --help to see options
# Copy the rpctest binary to all nodes into /root folder
# Edit this script and redefine half1 and half2 bash arrays with your IP addresses
...
for node in "${half1[@]}"; do
  ...
  ssh -n $node /root/rpctest -server &
done
...
for node in "${half2[@]}"; do
  #ssh $node 'echo $[4*1024] $[1024*1024] $[4*1024*1024] | tee /proc/sys/net/ipv4/tcp_wmem > /proc/sys/net/ipv4/tcp_rmem'
  #ssh -n $node "/root/iperf -c ${half1[$i]} -t 30 -i3 > iperftest.log" & iperf alternative test
  #ssh -n $node "/root/iperf -c ${half1[$i]} -t 30 -i3 -w 16K # 16K socket buffer/window size MapR uses
  ...
  ssh -n $node "/root/rpctest -client 5000 ${half1[$i]} > rpctest.log" & # Concurrent mode, comment out if using sequential mode
  ((i++))
done
